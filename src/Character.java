...
public class Character extends CharacterTemplate implements Serializable {
    ...
    public boolean containArtifact(String name) {
    	for(ArtifactTemplate a : artifacts) {
    		if(a.getName().compareTo(name) == 0) return true;
    	}
    	return false;
    }
    ...
    public static final class Builder {
        ...
        public Character build() {
        	if(hp <= 0) throw new IllegalStateException("Hp can't be <= 0");
        	if(attack <= 0) throw new IllegalStateException("Attack can't be <= 0");
        	if(defence <= 0) throw new IllegalStateException("Defense can't be <= 0");
